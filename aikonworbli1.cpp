...
CONTRACT aikonworbli1 : contract, public createaccounts{
private:
    Registry dapps;
    Balances balances;
    Token token;
...
public:
    ...
    aikonworbli1(name receiver, name code,  datastream<const char*> ds):contract(receiver, code, ds),
...
void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    ...
    if( code == self ) switch(action) {
        EOSIO_DISPATCH_HELPER( aikonworbli1, (init)(clean)(create)(define)(whitelist)(reclaim))
    }

    else {
        if(code == name("eosio.token").value && action == name("transfer").value){
            execute_action(name(receiver), name(code), &aikonworbli1::transfer);
        }
    }
}
};